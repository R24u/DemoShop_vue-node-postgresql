# Kushop - E-Commerce

Vue 3 + Express.js e-commerce app

## TEST USER

-Email : test123@gmail.com
-password : test123

## ✅ Done
- **Backend**: Express API, PostgreSQL, JWT, Multer uploads, routes for products/members/carts
- **Frontend**: Vue 3 + Vite, Router, Pinia stores, Bootstrap 5, 8+ pages

## 🚧 TODO
- API service layer, error handling, validation, auth middleware
- Component implementation, form validation, state persistence
- API docs, tests, Docker, CI/CD

## 🚀 Setup

### Backend
```bash
cd back_end && npm install && npm start  # Port 3000
```
.env: PORT=3000, DBSERVER=postgres, DBUSER=dev, DBPWD=1234, DBHOST=localhost, DBPORT=5432, DB=kushop

### Frontend
```bash
cd front_end && npm install && npm run dev  # Port 5173
```

